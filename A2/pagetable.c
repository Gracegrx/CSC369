...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (victim->frame & PG_DIRTY)
		{
			evict_dirty_count++;
			int offset_new;			
			offset_new = swap_pageout(frame, victim->swap_off);
			assert(offset_new != INVALID_SWAP);
			victim->swap_off = offset_new;
		}
		else
		{
			evict_clean_count++;
