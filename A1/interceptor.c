...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	{
		int monitored = table[sysc].monitored;
		//printk(KERN_DEBUG "Debug:");
		if (monitored == 2 || (monitored == 1 && check_pid_monitored(sysc, pid)))
		{
			log_message(pid, sysc, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	else if (cmd == REQUEST_SYSCALL_RELEASE)
	{
		if (current_uid() == 0)
		{
			...
			if (!table[syscall].intercepted || pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)
			{
				return -EINVAL;
			}
			else
